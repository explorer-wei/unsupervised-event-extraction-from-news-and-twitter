 ...
 * 
 */
public class LDAonNewsBodyClusters {
	public static List<Document>[] clusterNews(String newsFileDir){
		List<Document>[] array = new List[ClusterConstants.CLUSTER_NUM];
		for(int i=0;i<array.length;i++){
			array[i]  = new ArrayList<Document>();
		}

		List<Document> documentList = DocumentProcess.createDocumentsFromNews(newsFileDir);
	...
	public static void applyLDA(String LDARootDir, List<Document>[] docListArray){
		...
		for(List<Document> list : docListArray){
			try {
				...
				String[] args = {"-est", "-ntopics", "10", "-twords", "7", "-dir", ldaDir, "-dfile","LDA.txt"};
				...
				LDAUtility.produceLDAFileFromTitles(list, ldaFileUrl);
...
/*		LDACmdOption option = new LDACmdOption();
		...
		LDAUtility.translateLDAResult(option.dir + File.separator
				...
				+ "result.txt", Integer.parseInt(args[4]));*/
		String newsFileDir = "D:\\Test\\UkraineCrisis";
		String LDARootDir = "D:\\LDA";
		
		List<Document>[] array = clusterNews(newsFileDir);
